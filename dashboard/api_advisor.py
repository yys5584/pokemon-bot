...
- **⚠️ 팀 코스트 제한 (매우 중요!)**: 6마리 팀 총 코스트 합이 **12 이하**여야 하고, **반드시 6마리**를 채워야 함
  - 일반(1코) / 레어(2코) / 에픽(4코) / 전설(5코) / 초전설(6코)
  - 12코스트 + 6마리 주요 조합:
    - 에픽2(8) + 일반4(4) = 12 ✅
    - 에픽1(4) + 레어3(6) + 일반2(2) = 12 ✅
    - 에픽1(4) + 레어2(4) + 일반3(3) = 11 ✅
    - 전설1(5) + 레어2(4) + 일반3(3) = 12 ✅
    - 초전설1(6) + 레어1(2) + 일반4(4) = 12 ✅
    - 초전설1(6) + 일반6(6) = 12 → ❌ 7마리! 6마리로 맞출 것
  - 에픽 3마리(12코)는 나머지 0코로 6마리 불가 → **에픽은 최대 2마리**
  - 초전설+전설(11코)은 나머지 1코로 4마리 불가 → **초전설 쓰면 전설/에픽 불가**
  - 팀 추천 시 반드시 코스트 합계를 계산하고 12 이하 + 6마리인지 확인할 것
...
### 추천 우선순위 규칙 (매우 중요 — 12코스트 기준)
- **12코스트 + 6마리 필수** → 고등급만으로는 6마리 불가. 레어/일반도 적극 추천!
- 초전설(6코) 쓰면 나머지 5마리를 6코 이내로 → 레어1+일반4 또는 일반5+레어0 등
- 전설(5코) 쓰면 나머지 5마리를 7코 이내로 → 레어1+일반4 또는 레어2+일반2+일반1 등
- 에픽(4코)은 최대 2마리(8코), 나머지 4마리를 4코(일반4)로 채움
- **레어/일반이 핵심 전력**이 되는 환경:
  - 이로치 레어/일반은 친밀도7, IV보정으로 에픽급 스탯 가능 → 적극 추천
  - 타입 상성 카운터로 저코스트 포켓몬 활용 (예: 페어리 일반으로 드래곤 견제)
  - 높은 IV(S~A급) 레어는 낮은 IV 에픽보다 강할 수 있음
- **⚠️ 팀 추천 시 반드시 코스트 합계를 표시하고 12 이하 + 6마리인지 검증할 것!**
