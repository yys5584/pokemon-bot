...
async def check_and_unlock_titles(user_id: int) -> list[tuple[str, str, str]]:
    ...
    counts = await pool.fetchrow("""
        SELECT
            ...
            (SELECT COUNT(DISTINCT pokemon_id) FROM pokedex WHERE user_id = $1 AND pokemon_id BETWEEN 387 AND 493) AS pokedex_gen4,
    ...
    for title_id, (name, emoji, desc, check_type, threshold) in config.UNLOCKABLE_TITLES.items():
        ...
        elif check_type == "pokedex_gen4":
            unlocked = counts["pokedex_gen4"] >= threshold
