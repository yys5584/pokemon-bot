...
### 버그 수정 절차
- 버그 수정 시 **재현 테스트 먼저 작성** → 수정 → 전체 테스트 통과 확인
- `python -m pytest tests/ --tb=short -q` 통과 전까지 배포 금지
- 테스트 작성이 불가능한 경우(텔레그램 API 의존 등)에만 예외, 사유 명시

### 금지
- **배포/푸시/머지를 사용자 확인 없이 절대 하지 말 것**
- **봇 재시작을 사용자 확인 없이 절대 하지 말 것**
- `config.py`의 이모지 ID를 임의로 변경하지 말 것
- **`git add -A` 또는 `git add .` 사용 금지** — 변경한 특정 파일만 `git add`

### 문서화
- 새 기능/큰 수정 완료 후 `docs/specs/{기능명}.md`에 최종 구현 문서 작성
- 템플릿: `docs/specs/README.md` 참조
