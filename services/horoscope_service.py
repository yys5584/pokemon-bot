...
# ── 도수 구간 (같은 별자리라도 날마다 다른 뉘앙스) ──
_DEGREE_PHASES = {
    "early": {"range": "0~10°", "label": "초반",
              "nuance": "새로운 시작의 에너지, 미숙하지만 신선한 충동, 잠재력이 깨어남"},
    "mid":   {"range": "10~20°", "label": "중반",
              "nuance": "가장 안정된 발현, 본질이 또렷하게 드러남, 일관된 힘"},
    "late":  {"range": "20~30°", "label": "후반",
              "nuance": "전환기의 긴장, 다음 단계를 준비하는 마무리, 성찰과 과잉이 공존"},
}
...
def _get_degree_phase(degree: float) -> dict:
    """도수 → 초반/중반/후반 구간 정보."""
    if degree < 10:
        return _DEGREE_PHASES["early"]
    elif degree < 20:
        return _DEGREE_PHASES["mid"]
    else:
        return _DEGREE_PHASES["late"]


def _get_moon_phase(target_date: date) -> dict:
    """달 위상(음력) 계산 — swisseph 기반."""
    try:
        import swisseph as swe
        swe.set_ephe_path(None)
    except ImportError:
        return {"name": "알 수 없음", "emoji": "🌙", "meaning": ""}
    ...
    if phase_angle < 22.5:
        return {"name": "신월(삭)", "emoji": "🌑", "meaning": "씨앗을 뿌리는 시기, 내면 집중, 새로운 의도 설정"}
    elif phase_angle < 67.5:
        return {"name": "초승달", "emoji": "🌒", "meaning": "결단과 추진, 장애물 돌파, 행동으로 옮기는 시기"}
    elif phase_angle < 112.5:
        return {"name": "상현달", "emoji": "🌓", "meaning": "긴장과 조정, 방향 수정이 필요한 시점, 균형 잡기"}
    elif phase_angle < 157.5:
        return {"name": "볼록달(상)", "emoji": "🌔", "meaning": "기대와 완성 직전, 세부 조율, 인내가 빛을 발함"}
    elif phase_angle < 202.5:
        return {"name": "보름달(망)", "emoji": "🌕", "meaning": "절정과 깨달음, 감정 고조, 결실과 해방"}
    elif phase_angle < 247.5:
        return {"name": "볼록달(하)", "emoji": "🌖", "meaning": "나눔과 감사, 얻은 것을 소화, 타인에게 베풂"}
    elif phase_angle < 292.5:
        return {"name": "하현달", "emoji": "🌗", "meaning": "비움과 정리, 불필요한 것을 놓아줌, 반성"}
    elif phase_angle < 337.5:
        return {"name": "그믐달", "emoji": "🌘", "meaning": "휴식과 치유, 에너지 충전, 조용한 내면 여행"}
    else:
        return {"name": "신월(삭)", "emoji": "🌑", "meaning": "씨앗을 뿌리는 시기, 내면 집중, 새로운 의도 설정"}
...
def _build_transit_text(transits: dict, target_date: date) -> str:
    """트랜짓 데이터를 AI 프롬프트용 텍스트로 변환 (도수 구간 + 달 위상 포함)."""
    lines = ["[오늘의 행성 배치]"]
    for p in transits["planets"]:
        phase = _get_degree_phase(p["degree"])
        lines.append(f"{p['symbol']}{p['name']}: {p['sign']}{p['sign_symbol']} {p['degree']}° ({phase['label']}·{phase['nuance']})")
    if transits["aspects"]:
        lines.append(f"\n[주요 어스펙트] {', '.join(transits['aspects'][:8])}")

    # 달 위상
    moon_phase = _get_moon_phase(target_date)
    lines.append(f"\n[달 위상] {moon_phase['emoji']} {moon_phase['name']}: {moon_phase['meaning']}")

    # 요일 에너지
    weekday = target_date.weekday()
    wd_name, wd_energy = _WEEKDAY_ENERGY[weekday]
    lines.append(f"[요일] {wd_name} — {wd_energy}")
...
## 일일 변화 원칙 (매우 중요!)
- 행성의 '도수 구간'(초반/중반/후반)과 '달 위상'을 핵심 변화 요인으로 활용하세요.
- 달 위상이 운세의 감정 톤과 행동 방향을 결정합니다 (신월→내면, 보름달→절정/감정 고조 등).
- 같은 행성-별자리 조합이어도 도수에 따라 해석이 달라야 합니다.
- 요일 에너지도 서술의 구체적 맥락(시작/행동/소통/확장/조화/정리/회복)에 반영하세요.
- 매일 다른 관점·비유·문체를 사용하세요. 어제와 비슷한 구조나 단어 조합을 반복하지 마세요.
- 특히 "고통에 공감", "에너지를 승화" 같은 상투적 표현을 날마다 쓰지 마세요.
...
async def _generate_horoscope_ai(sign: dict, transits: dict, target_date: date) -> str | None:
    ...
    transit_text = _build_transit_text(transits, target_date)
    ...
    # 태양 도수 구간
    sun_data = next((p for p in transits["planets"] if p["name"] == "태양"), None)
    sun_phase = _get_degree_phase(sun_data["degree"]) if sun_data else _DEGREE_PHASES["mid"]

    user_prompt = f"""오늘 날짜: {target_date} ({wd_name})
별자리: {sign['name']} ({sign['symbol']}, {sign['en']})
원소: {sign['element']} | 지배행성: {ruler} | 모드: {mode}
성격 키워드: {trait}
취약 부위: {body}

## 오늘의 핵심 변수 (어제와 다른 운세를 만드는 열쇠)
- 달 위상: {moon_phase['emoji']} {moon_phase['name']} → {moon_phase['meaning']}
- 태양 도수: {sun_phase['label']}({sun_phase['range']}) → {sun_phase['nuance']}
- 요일: {wd_name} → {wd_energy}
...
달 위상({moon_phase['name']})의 에너지가 운세의 감정 톤을 결정합니다.
다른 별자리 운세와 절대 비슷해지면 안 됩니다. 어제와 같은 표현/구조도 금지합니다."""
